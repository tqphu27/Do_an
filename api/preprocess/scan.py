...
import sys
sys.path.append('/home/tima/Do_an/api/preprocess')
...
class DocScanner(object):
    ...
    def scan(self, image, output_path=None, binary=False):
        ...
        # plt.imshow(image)
        # plt.show()
        rescaled_image = cv2.resize(orig, (int((h*int(RESCALED_HEIGHT)/w)),int(RESCALED_HEIGHT)))
...
def scan_img(image):
    ...
    img = scanner.scan(image)

    return img


if __name__ == "__main__":
    scan_img(cv2.imread('/home/tima/imageimage/download (28).jpeg'))
    # print(img.shape)
